...
export function CinematicGoldTemplate({ surprise, onEvent }: TemplateProps) {
  ...
  const accent = surprise.accentColor || TOKENS.gold;
  const displayName = surprise.nickname?.trim() || surprise.recipientName;
  const confettiCount = Math.round(
    60 * (INTENSITY_MULTIPLIER[surprise.animationStyle] ?? 1),
  );
      ...
      <AnimatePresence mode="wait">
        {scene === "intro" && (
          ...
          >
            ...
            <button
              ...
              style={{
                background: `linear-gradient(135deg, ${accent}, ${TOKENS.goldDeep})`,
                color: "#1B1330",
                boxShadow: `0 10px 30px -8px ${accent}77`,
              }}
            >
              Open My Surprise 🎁
            </button>
          ...
          >
            ...
            {surprise.specialMemory && (
              <motion.p
                ...
                className="mt-4 max-w-xs text-sm italic"
                style={{ color: TOKENS.cream, opacity: 0.75 }}
              >
                &quot;{surprise.specialMemory}&quot;
              </motion.p>
            )}
        ...
        {scene === "final" && (
          ...
          >
            ...
            {surprise.quote && (
              <p
                className="mt-4 max-w-xs font-serif text-base italic"
                style={{ color: TOKENS.gold }}
              >
                {surprise.quote}
              </p>
