...
export function CinematicGoldTemplate({ surprise, onEvent }: TemplateProps) {
  ...
  return (
    <div
      ...
      style={{ background: TOKENS.bg, color: TOKENS.cream, fontFamily: "var(--font-outfit, sans-serif)" }}
    >
      <LightRays color={TOKENS.gold} />
      <ConfettiBurst trigger={burst} colors={TOKENS.confetti} />
      {surprise.musicUrl && (
        <MusicPlayer src={surprise.musicUrl} accentColor={TOKENS.gold} onStart={() => onEvent?.("music_started")} />
      )}

      <AnimatePresence mode="wait">
        {scene === "intro" && (
          <motion.div key="intro" {...fade} className="relative flex flex-col items-center">
            <p className="mb-4 text-xs uppercase tracking-[0.35em]" style={{ color: TOKENS.gold }}>
              a private premiere
            </p>
            <p className="max-w-xs text-lg leading-relaxed">
              Someone made something{" "}
              <span className="font-serif italic" style={{ color: TOKENS.gold }}>extraordinary</span>{" "}
              for you...
            </p>
            <button
        ...
        {scene === "reveal" && (
          ...
          >
            <p className="mb-3 text-xs uppercase tracking-[0.35em]" style={{ color: TOKENS.gold }}>
              happy birthday
            </p>
            <NameReveal
        ...
        {scene === "photo" && surprise.mainPhotoUrl && (
          <motion.div key="photo" {...fade} className="relative flex flex-col items-center">
            <PhotoReveal
            ...
            />
            <button onClick={() => advance("gallery")} className="mt-8 text-sm underline underline-offset-4" style={{ color: TOKENS.gold }}>
              Continue
            </button>
          </motion.div>
        )}
        ...
        {scene === "gallery" && surprise.memoryPhotos.length > 0 && (
          <motion.div key="gallery" {...fade} className="relative flex flex-col items-center" onAnimationComplete={() => onEvent?.("gallery_viewed")}>
            <p className="mb-6 text-xs uppercase tracking-[0.35em]" style={{ color: TOKENS.gold }}>a few memories</p>
            <MemoryGallery photos={surprise.memoryPhotos} />
            <button onClick={() => advance("message")} className="mt-8 text-sm underline underline-offset-4" style={{ color: TOKENS.gold }}>
              Continue
            </button>
          </motion.div>
        )}
        ...
        {scene === "message" && (
          <motion.div key="message" {...fade} className="relative flex flex-col items-center">
            <div className="max-w-md rounded-2xl border p-1" style={{ borderColor: `${TOKENS.gold}44` }}>
              <TypewriterMessage
            ...
            <button onClick={() => advance("age")} className="mt-6 text-sm underline underline-offset-4" style={{ color: TOKENS.gold }}>
              Continue
            </button>
          </motion.div>
        )}
        ...
        {scene === "age" && (
          <motion.div key="age" {...fade} className="relative flex flex-col items-center">
            <p className="mb-2 text-xs uppercase tracking-[0.35em]" style={{ color: TOKENS.gold }}>turning</p>
            <AgeReveal
            ...
            <button
              ...
              className="mt-8 rounded-full px-8 py-4 text-sm font-medium"
              style={{ background: `linear-gradient(135deg, ${TOKENS.gold}, ${TOKENS.goldDeep})`, color: "#1B1330" }}
            >
              One More Surprise 🎁
            </button>
          </motion.div>
        ...
        {scene === "final" && (
          <motion.div key="final" {...fade} className="relative flex flex-col items-center">
            <h1 className="font-serif text-4xl" style={{ color: TOKENS.gold }}>
              Happy Birthday, {surprise.recipientName} 🎂
            </h1>
            <h3 className="mt-2 font-serif text-2xl italic" style={{ color: TOKENS.cream }}>
